...
実機の遅い回線で「BGMのダウンロードが終わる前にゲームが始まり、音が1分遅れて鳴る」問題を修正しました。

- **ローディングがBGMの読み込み完了まで待つ**ようになり、STARTボタンは楽曲が再生可能になってから表示されます（「♪ 楽曲をダウンロード中… n%」とバイト単位の進捗を表示）
- **Cache Storage APIによる永続キャッシュ**（`adbreaker-assets-v1`）: BGM（5.8MB）と広告画像91枚は**一度ダウンロードすると端末に保存され、次回からはネットワークを使わず即読み込み**されます
- Cache APIが使えない環境（プライベートブラウズ等）では通常のダウンロードに、それも失敗した場合は従来のストリーミング再生に自動フォールバック（ゲームは止まりません）
- 副産物として、音声がblob化されたことでどの環境でもシークが可能になり、リロード時の `bgm.mp3 206` 中断も発生しなくなりました

> **運用メモ**: `bgm.mp3` を同名のまま差し替えた場合は `src/assetCache.js` の `CACHE_NAME` のバージョンを上げてください（広告画像はファイル名が変わるため自動で新規取得されます）。キャッシュはブラウザのサイトデータ削除で消去できます。

> **楽曲選択画面について**: 現在は1曲のみのため導入していません。複数曲対応時には「曲選択画面で選んだ曲だけをダウンロード」する構成が適しています。

### FEVER＝花火大会リニューアル（スコア仕様変更・leaderboard v2）

FEVERを「花火大会のフィナーレ」として再設計しました。

- **発生頻度を1曲1〜2回に調整**（ゲージ上昇量 `fever.gainBase` を4→0.65に。同色ストリーク倍率で早く貯まるので、狙うプレイヤーほど2回目・上級者は3回目に届く）。90%でのREACH演出はそのまま
- **FEVER中は背景が花火大会の夜空のように暗転**（バナー・ボタンは別レイヤーなので視認性は維持）
- **ボタンは金色の「★」**（通常の×と明確に区別）。**終了時に残った★は自動で赤青黄緑の×に変換**され、金色が場や色抽選に残るバグも修正
- **キル数で花火が進化**: 〜15キル=スターマイン（小玉連発）→ 16〜35キル=菊花（尾を引く大輪）→ 36キル〜=**しだれ柳**（金色の枝垂れ）。ときどき画面下端から打ち上げロケットも上がる
- **残り2.2秒でフィナーレ**: ロケット3発が空一面の大玉しだれ柳として開き、「ドーン」という低音とともにFEVERを締めくくる

> **重要**: この変更でスコードが旧仕様と比較不能になったため、`leaderboardId` を **`adbreaker_song001_v2`**、`gameVersion` を **3.1.0** に世代交代し、Security Rulesも更新・デプロイ済みです。旧v1ボードは閉鎖（読み書き不可）。ランク閾値も新経済に合わせて S:120万 / A:80万 / B:50万 / C:25万 に再調整しました。
