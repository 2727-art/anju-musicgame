...
同色の×印をタップし続けると、タップ位置が「星」になり輝くラインで順につながっていきます。**6個目の星で星座が完成**し、描いた軌跡の形をリサンプル＋正規化（重心・スケール・回転8方位・鏡像・描き順を吸収）して**最も形が近い12星座**を判定、「♏ さそり座 COMPLETE!!」と天球チャイムで祝福されます。色が変わる・Missするとラインは静かにフェードし、完成後は次の同色タップから新しい星座を描き始めます。リザルトに「星座コンプリート」数をローカル表示（**ランキングには送信しません**）。判定閾値は `config.js` の `constellation.starsToComplete`、星座形状は `src/zodiacMatcher.js` で調整できます。流れ星エフェクトOFFで星座も無効になります。
